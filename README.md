...
**Zsh**
```sh
new completion zsh > "${fpath[1]}/_new"
# Start a new shell session to activate, or:
source ~/.zshrc
```
...
# Or for a user install (no sudo):
new completion bash > ~/.local/share/bash-completion/completions/new
```
...
**PowerShell**
```sh
new completion powershell > $PROFILE.CurrentUserAllHostsProfile
```
